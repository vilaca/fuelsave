...
def test_create_token_no_body(test_app, monkeypatch):
    response = test_app.post("/token")
    assert response.status_code == 422


def test_create_token_no_username(test_app, monkeypatch):
    response = test_app.post("/token", data=json.dumps({"username": "user1"}))
    assert response.status_code == 422


def test_create_token_no_password(test_app, monkeypatch):
